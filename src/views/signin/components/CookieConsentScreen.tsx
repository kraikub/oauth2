...
export const CookieConsentScreen = () => {
  ...
  const acceptWithChoice = (c: string) => {
    setCookie("ACCEPT_COOKIES", c, { maxAge: appConfig.cookieMaxAge });
    location.reload();
  };
  ...
  return (
    <Box w="100%">
      <VStack spacing={3} alignItems="start">
        ...
        <Text size="md">{t("cookie-consent-description")}</Text>
      </VStack>
      <ButtonGroup justifyContent="end" w="full" my={5}>
        <Button onClick={() => acceptWithChoice("nesc")}>
          {t("cookie-consent-btn-nesc")}
        </Button>
        <Button onClick={() => acceptWithChoice("all")}>
          {t("cookie-consent-btn-all")}
        </Button>
      </ButtonGroup>
    </Box>
  );
